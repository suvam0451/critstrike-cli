...
func ParseHlslFile(inpath, outpath, codeprefix string) {
	...
	if textFile, err := os.Open(inpath); err == nil {
		...
		// This is the root material reference in python file
		matref := "root"
		...
		LinesToWrite = append(LinesToWrite, "\n# Attach custom expression node",
			fmt.Sprintf("%s = unreal.MaterialEditingLibrary.create_material_expression(%s, unreal.MaterialExpressionCustom, -400, 0)",
				firstCustom.GetID(), matref))
